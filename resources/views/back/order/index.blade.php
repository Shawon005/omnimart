        ...
        <form action="{{route('back.order.index')}}" method="GET">
          <div class="row mb-4 justify-content-center">
			...
			<div class="gd-responsive-table">
              ...
              <th>{{ __('Curier Agent') }}</th>
							<th>{{ __('Actions') }}</th>
						</tr>
					</thead>

					<tbody>
              @include('back.order.table',compact('datas','curier'))
					</tbody>
...
<div class="modal fade" id="curierAgentModal" tabindex="-1" role="dialog" aria-labelledby="curierAgentModalModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">

		<!-- Modal Header -->
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">{{ __('Update Curier Agent?') }}</h5>
          <button class="close" type="button" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">×</span>
          </button>
		</div>
		...
		<!-- Modal Body -->
      <div class="modal-body">
			    {{ __('You are going to update the curier agent.') }} {{ __('Do you want proceed?') }}
		  </div>
