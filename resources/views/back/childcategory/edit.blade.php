						...
						<div class="col-lg-12">
								<form class="admin-form" action="{{ route('back.childcategory.update',$childcategory->id) }}" method="POST"
									...
									@php
										...
										$childcategory->load('translations');
									@endphp
									...
									<div class="tab-content">
										@foreach($languages as $index => $lang)
										@php
											$translation = $childcategory->translations->where('language_id', $lang->id)->first();
										@endphp
										<div class="tab-pane fade {{ $index === 0 || $lang->id == $defaultLang->id ? 'show active' : '' }}" 
											...
											<div class="form-group">
												...
												<input type="text" name="name_{{ $lang->id }}" class="form-control item-name {{ $lang->id == $defaultLang->id ? 'slug-source' : '' }}" 
													...
													value="{{ $translation ? $translation->name : ($lang->id == $defaultLang->id ? $childcategory->name : '') }}" >
											</div>
											...
											<div class="form-group">
												...
												<input type="text" name="slug_{{ $lang->id }}" class="form-control slug-input" 
													...
													value="{{ $translation ? $translation->slug : ($lang->id == $defaultLang->id ? $childcategory->slug : '') }}" >
											</div>
											@if($lang->id == $defaultLang->id)
												<input type="hidden" name="name" value="{{ $childcategory->name }}">
